...
# ISA for grouped treatments
isa_ms <- multipatt(t(otu_table(ms_genus_RA)), cluster = sample_data(ms_genus_RA)$`treatment_status`)
summary(isa_ms)
taxtable <- tax_table(ms_rare_no_RRMS_ctrl) %>% as.data.frame() %>% rownames_to_column(var="ASV")
...
#save table to github
write.csv(res, "ISA_grouped_results.csv", row.names = FALSE)                                       
